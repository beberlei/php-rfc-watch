    ...
    render() {
        ...
        return <AppContext.Consumer>
            ...
            <span className="relative z-0 inline-flex shadow-sm">
                ...
                <TailwindButton className={"rounded-r-md -ml-px"} onClick={() => this.vote(-1, ctx.logged_in)} title={!ctx.logged_in ? "Login required" : ""} disabled={!ctx.logged_in}>
                    <svg className={"h-5 w-5"} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2"
...
class TailwindButton extends React.Component {
    render() {
        ...
        if (!this.props.disabled) {
            classes = classes + " text-gray-500 hover:text-gray-400 focus:z-10 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150"
        } else {
            classes = classes + " text-gray-400"
        }
